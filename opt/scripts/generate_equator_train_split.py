...
outfilepath = "/home/ccl/Datasets/NeRF/ScanNerf/airplane1/train_equator_25.json"
numberOfImg = 25 
valueOfZ = 0 
...
def findClosestImages(filepath, num_points, z):
...
lis = findClosestImages(filepath,numberOfImg,valueOfZ)

#write to json file 
#"{'camera_angle_x': 0.8836704333486497, 'camera_angle_y': 0.6819013496909203, 'fl_x': 1522.1201085541113, 'fl_y': 1521.954743529035,\n 'k1': 0.0652210706564525, 'k2': 0.23102137964696204, 'p1': -0.0018667704274258755, 'p2': -0.00035823757611365637,\n 'cx': 727.9348613007779, 'cy': 541.5426465751151, 'w': 1440, 'h': 1080, 'aabb_scale': 1, 'frames': [\n"
jsonfile = """{
    "camera_angle_x": 0.8836704333486497,
    "camera_angle_y": 0.6819013496909203,
    "fl_x": 1522.1201085541113,
    "fl_y": 1521.954743529035,
    "k1": 0.0652210706564525,
    "k2": 0.23102137964696204,
    "p1": -0.0018667704274258755,
    "p2": -0.00035823757611365637,
    "cx": 727.9348613007779,
    "cy": 541.5426465751151,
    "w": 1440,
    "h": 1080,
    "aabb_scale": 1,
    "frames": [ """

#add all the different frames to the same string, with the correct formatting 
count = 0 
for x in lis:
    res = x.replace("'", '"')
    if(count < (len(lis) - 1)): 
...
#parse string into json file 
jfile = json.loads(jsonfile)
with open(outfilepath , 'w') as fout:
    json.dump(jfile, fout)
